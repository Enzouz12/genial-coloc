...
import { averageScore, formatScore, scoreColor } from "../config";
import {
  uploadMedia,
  uploadBlob,
  signedUrl,
...
/** Corps d'édition des notes structurées d'une annonce (visite, contacts,
 *  liens, médias, avis, notes). Utilisé par la page /offre/:id. */
export function OfferDetailEditor({ offer, me, onSave }: Props) {
  ...
  // Quitter sans enregistrer (démontage) → nettoie les fichiers téléversés.
  // On lit volontairement la valeur courante du ref au démontage (pas à l'init).
  useEffect(() => {
    const session = sessionPaths.current;
    const saved = savedRef;
    return () => {
      if (!saved.current && session.size) void deleteMedia([...session]);
    };
  }, []);
